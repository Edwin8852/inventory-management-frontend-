...
const SupplierInvoices = () => {
  ...
  const generatePDF = (invoice) => {
    ...
    doc.text('PURCHASE INVOICE', pageWidth - margin, yPos, { align: 'right' });
    ...
    autoTable(doc, {
      startY: yPos,
      head: [['Product Description', 'Cost Price', 'Bulk Qty', 'Total amount']],
      body: tableData,
      theme: 'striped',
    ...
    yPos += 6;
    doc.text('Supplier GST:', totalsX, yPos);
    ...
    doc.save(`Purchase_Invoice_${invoice.invoiceNumber}.pdf`);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">My Purchase Invoices</h1>
          <p className="text-sm text-gray-500">View and download your bulk purchase invoices.</p>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredInvoices.map((invoice) => (
                  <tr key={invoice.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4"><span className="px-2 py-1 bg-green-50 text-green-600 rounded text-xs font-bold">{invoice.status}</span></td>
                    <td className="p-4 text-right font-bold text-gray-900">{formatCurrency(invoice.totalAmount)}</td>
                    <td className="p-4 flex justify-center gap-2">
                      <button 
                        onClick={() => setViewingInvoice(invoice)}
                        className="flex items-center text-xs px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg transition-colors font-medium"
                      >
                        <FileText className="w-3 h-3 mr-1" /> View UI
                      </button>
                      <button 
